...
# "news" is intentionally INCLUDED.
...
def sorted_immediate_entries(folder: Path, channel_root: Path):
    ...
    # For channel root, force important files first in this exact order:
    # UniExtract.exe, news, then all other files alphabetically.
    if folder == channel_root:
        ...
        # ordered may contain only UniExtract.exe, or UniExtract.exe then news,
        # or just news if UniExtract.exe is missing (should not happen for channels).
...
def main() -> None:
    ...
    print("- news is included if present")
